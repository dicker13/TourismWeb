      ...
      <?php foreach($destinasi->result_array() as $row){ ?>
          <div class="col-sm-4 col-md-4">
            <div class="thumbnail" style="margin-top: 20px;margin-bottom: 40px;">
              <h4 class="judul"><?php echo $row['judul']; ?></h4>
            <img src="<?php echo base_url('assets'); ?>/uploads/files/<?php echo $row['gambar']; ?>" alt="gambar1">
            <div class="caption">
              <p><?php echo character_limiter(strip_tags($row['isi']), 200); ?></p>            
              <p style="float:right;"><a href="<?php echo base_url('destinasi/detail').'/'.$row['id_destinasi']; ?>" class="btn btn-default" role="button">Selengkapnya</a></p>
              <br/><br/>
            </div>
            </div>
          </div>
      <?php } if($destinasi->num_rows() < 1){ echo '<h4>Data tidak ditemukan!</h4>'; } ?>
